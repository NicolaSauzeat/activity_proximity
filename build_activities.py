...
for i in range(0, len(nomenclature_NACE2)):
    # Sélectionne le premier code NACE
	NACE_code = nomenclature_NACE2[i,0]
	print('code = ', NACE_code)
    # Enregistre le nombre de correspondance HS
	...
	if (nb_NACE_code > 0):
        # Création d'une matrice de 100 colonnes et d'un nombre de ligne égale au nombre de correspondances
		tab_HS_vectors = np.zeros((nb_NACE_code, var_dimension))
        # Identifie le poids de correspondance entre HS et NACE
		tab_HS_weights = np.zeros(nb_NACE_code)
		print("trouvé, len=",nb_NACE_code)
		for j in range(0, nb_NACE_code):
            # Identifie le code HS lié au compte NACE
